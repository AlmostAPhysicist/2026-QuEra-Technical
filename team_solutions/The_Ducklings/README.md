...
qec/                          # Core QEC package
├── states.py               # Clifford state generators (|0⟩, |+⟩, etc.)
├── encoding.py             # MSD encoding circuit
├── syndrome.py             # Syndrome measurement kernels
├── errors.py               # Error injection
├── error_mapping.py        # Classical decoder (syndrome → error location)
├── correction.py           # Full QEC pipeline
└── logical_ops.py          # Logical operations

demo0.py                     # Single QEC cycle (customizable)
demo1.py                     # Full benchmark (4 noise levels)
qec_demo.ipynb              # Interactive notebook (Recommended)
run_qec_alternative.py      # Standalone Python script
```
...
jupyter notebook qec_demo.ipynb
```
- Single QEC cycle with circuit visualization
- 4-noise benchmark (0%, 5%, 25%, 60%)
- Power-law analysis with plots
- Summary table
...
python run_qec_alternative.py
```
Same as notebook, but with ENTER prompts between sections. Saves PNG plots.

### Option 3: Quick Single Test
```bash
python demo0.py     # Single cycle (edit parameters in file)
python demo1.py     # Full benchmark
```
...
**At 25% physical noise:**
- Baseline: 92.8% fidelity
- Postselection: 100% fidelity (but waste 25% of runs)
- **Active Correction: 99.2% fidelity (no waste)** ✅

**Power-law scaling (50% noise):**
- Baseline improves as N^(-0.5)
- Postselection plateaus
- **Active Correction improves as N^(-1)** → 5x better at 100 shots

---

## Challenge Status

✅ **Step 1:** Bloqade kernel definition, PyQrack simulation
✅ **Step 2:** Steane QEC, MSD circuit, syndrome extraction, noise analysis, power-law plotting
⏳ **Bonuses:** Distance-5, recurrent correction, atom moving, Tsim backend (not implemented)

---

## References

1. Steane (2007) - Active stabilizer fault tolerance
2. QuEra (2024) - [arxiv:2412.15165](https://arxiv.org/abs/2412.15165) - Magic State Distillation
3. Color Code & Distance-5 - [arxiv:2312.03982](https://arxiv.org/pdf/2312.03982), [arxiv:2601.13313](https://arxiv.org/pdf/2601.13313)

---

**Team:** The Ducklings  
**Status:** Complete (Core + Analysis) — February 1, 2026
